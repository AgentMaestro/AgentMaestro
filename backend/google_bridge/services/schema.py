...
GOOGLE_BRIDGE_QUERY_LANGUAGE_FIELD_SUPPORT = (
    "Supported query and payload fields vary by surface: Gmail list/read currently uses from, to, subject, label_ids, in, is, newer_than, and older_than in the query string; Gmail read-by-id can also request message bodies and MIME parts by passing format=full plus include_body/include_html/include_attachments, or format=raw when the RFC822 source is needed and will be decoded server-side; Gmail write and cleanup flows also accept message_id when targeting a single message. "
...
GOOGLE_BRIDGE_TOOL_DESCRIPTION = (
    ...
    "For Gmail sweeps that fan out across many messages or multiple connected accounts, spawn a subrun or child and reclaim the result later instead of keeping the interactive turn blocked. "
