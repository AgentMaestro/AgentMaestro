...
from ..config import PYTHON_INTERPRETER, policy_runtime_root, resolve_policy_path
...
def run_python(
    ...
    execution_dir = policy_runtime_root(policy, "repo_root") or run_dir.resolve()
    if args.files:
        for file in args.files:
            target = resolve_policy_path(run_dir, file.path, policy)
            target.parent.mkdir(parents=True, exist_ok=True)
            target.write_bytes(base64.b64decode(file.content_b64))
    if args.entrypoint:
        main_file = resolve_policy_path(run_dir, args.entrypoint, policy)
